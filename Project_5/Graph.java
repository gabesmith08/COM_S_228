...
public class Graph {
	...
	public void addEdge(int fromIdx, int toIdx, int index, String word)
		...
		// TODO complete
		if (fromIdx < 0 || fromIdx > vertices.length -1 || toIdx < 0 || toIdx > vertices.length -1) {
	...
	public int[] fillGArray(int words) {
		...
		for (Vertex v : vertices) { // iterate through vertices and fill array 
			if (!v.isVisited()) {
				toRet[v.index()] = 0;
	...
	public boolean hasCycle() {
		...
		for(int i = 0; i < vertices.length; i++) {
			...
			{
				if(vertices[i].hasCycle(vertices[i])) {
