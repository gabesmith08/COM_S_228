...
public class GrahamScan extends ConvexHull
{
	...
	{	// TODO probably not correct
		...
		{
			Point middle = vertexStack.pop();
			Point first = vertexStack.peek();  
			Point last = pointsNoDuplicate[i];
			PolarAngleComparator comp = new PolarAngleComparator(first, false);
				if(comp.compare(middle, last) == 1) // if crossproduct between points is positive, take out middle point 
				{
					
					while(comp.compare(middle, last) == 1 && vertexStack.size() > 1)
					{
						middle = vertexStack.pop();
						first = vertexStack.peek();
						comp = new PolarAngleComparator(first, false);
					}
					vertexStack.push(last);
				}
				else
				{
					vertexStack.push(middle);
					vertexStack.push(last);
				}
		
		}
		hullVertices = new Point[vertexStack.size()];
		for(int i = vertexStack.size() -1; i >= 0; i--) // reverse iterate through hull vertices array and fill it with vertex stack points
		{
