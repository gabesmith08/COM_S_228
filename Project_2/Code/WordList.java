...
//import edu.iastate.cs228.hw2.Alphabet.CharAndPos;
...
public class WordList implements Cloneable {
	...
	public WordList(String filename) throws NullPointerException, FileNotFoundException {
		File file = new File(filename); // grabs file for scanner to scan through
		Scanner scan = new Scanner(file); // scanner to scan for length to initiate array length
		int length = 0;
	...
	public void swap(int idxA, int idxB) throws IndexOutOfBoundsException {
		String temp = ""; // just a basic swap method with a temporary variable that holds value to perform swap
		temp = words[idxA];
